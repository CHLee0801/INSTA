...
def check_args(config, checkpoint_path_through_command, dataset_path_through_command, target_cluster, target_dataset, is_bigbench, is_bbh, is_super):
    ...
    if 'is_bigbench' not in config:
        config.is_bigbench = is_bigbench
...
def training_run(args):
    ...
    if args.mode == 'train':
        ...
        if args.is_bigbench == True:
            if args.target_dataset == '':
                eval_path_lists = []
                base_path = 'data/bigbench_eval/'
                ...
                eval_path_lists = []
                base_path = f'data/bigbench_eval/{args.target_dataset}/'
                ...
                eval_path_lists = []
                path = f'data/bigbench_eval/{args.target_dataset}/test'
                eval_path_lists.append(path)
                eval_datasets.append(load_from_disk(path))
        else:
            eval_path_lists = []
            folder_path = 'data/p3_eval'
            if args.target_cluster != '':
                cluster_list = [f'data/p3_eval/{args.target_cluster}']
            else:
                cluster_list = ['data/p3_eval/'+f for f in os.listdir(folder_path)]
...
def main():
    ...
    parser.add_argument('--is_bigbench', action='store_true')
    ...
    config = check_args(argparse.Namespace(**config), arg_.checkpoint_path, arg_.dataset_path, arg_.target_cluster, arg_.target_dataset, arg_.is_bigbench, arg_.is_bbh, arg_.is_super)
