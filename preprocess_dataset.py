...
def load_train_dataset(dataset, config, tokenizer, mode):
    ...
    for row in dataset:
        tmp_dict['source'].append(str(row['context']))
        tmp_dict['target'].append(str(row['text']))
...
# exit()
# if 'eval' in config.dataset_path:
...
#         test_dataset.save_to_disk(config.dataset_path)
# else:
train_file = f"/home/ubuntu/ex_disk/MT_Selector/data/chatex_ko/sandbox_ver240429_4k.jsonl"
train_csv_file = load_dataset("json", data_files=train_file)['train']
