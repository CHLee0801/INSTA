...
def training_run(args):
    ...
    if args.mode == 'train':
        ...
        # train_dataset = load_from_disk(os.path.join("/home/ubuntu/ex_disk/MT_Selector/data/chatex", "train"))
    else:
        if args.is_bigbench == True:
            ...
            if args.target_dataset == '':
                ...
                eval_path_lists = []
                for path in eval_temp_list:
                    if '.py' in path or 'original' in path or 'word_sorting' in path:
                        continue
                    eval_path_lists.append(path)
                    eval_datasets.append(load_from_disk(path))
            else:
                eval_path_lists = []
                path = f'data/bbh/{args.target_dataset}/test'
                eval_path_lists.append(path)
                eval_datasets.append(load_from_disk(path))
        else:
            eval_path_lists = []
